  ...
  "AdminController" should {
    ...
    "be able to view assignments" in {
      ...
      val fakeRequest: FakeRequest[AnyContentAsFormUrlEncoded] = FakeRequest("GET", "/").withSession(
        "email" -> "ayush.tiwari@knoldus.in").withFormUrlEncodedBody("title" -> "title", "description" -> "desc")
      when(userRepository.isAdmin("ayush.tiwari@knoldus.in")).thenReturn(Future.successful(true))
    ...
    "be able to enable or disable users with success" in {
      when(userRepository.enableOrDisableUser("ayush.tiwari@knoldus.in", value = true)).thenReturn(Future.successful(true))
      val result: Future[Result] = adminController.enableOrDisableUser("ayush.tiwari@knoldus.in", isEnabled = false)
        .apply(FakeRequest("GET", "/"))
      redirectLocation(result) mustBe Some("/viewusers")
    }
    "fail to enable or disable users with success" in {
      when(userRepository.enableOrDisableUser("ayush.tiwari@knoldus.in", value = true)).thenReturn(Future.successful(false))
      val result: Future[Result] = adminController.enableOrDisableUser("ayush.tiwari@knoldus.in", isEnabled = false)
        .apply(FakeRequest("GET", "/"))
      status(result) mustBe 500
    }
